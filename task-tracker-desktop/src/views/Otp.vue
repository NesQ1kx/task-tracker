<template>
  <div class="d-flex justify-center align-center otp">
      <div class="d-flex flex-column align-center content">
        ...
        <div class="mb-3 subtitle-2 text-center">На вашу почту был выслан код подтверждения. Введите его ниже</div>
        <div>
          <v-otp-input
            :disabled="disabled"
            length="6"
  ...
  methods: {
    async onCompleteOtp(otpValue) {
      try {
        ...
        this.disabled = true;
        const response = await this.$http.post('auth/confirm', { code: otpValue });
